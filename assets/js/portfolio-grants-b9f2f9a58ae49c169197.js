...
// require('slider-before-after/src/scripts/slider-before-after');
...
function setup(options) {
    ...
    var config = _extends({}, DEFAULT_CONFIG, options);
    var slider = config.el;
    var track = slider.querySelector('.slider-track');
    var after = slider.querySelector('.slider-after-image');
    var afterImg = slider.querySelector('.slider-after-image img');

    // Copy styles.
    slider.style.width = getComputedStyle(afterImg).getPropertyValue('width');
    slider.style.height = getComputedStyle(afterImg).getPropertyValue('height');

    track.addEventListener('input', function (evt) {
        var percent = Number(evt.target.value);
...
function autoRun() {
    var sliders = document.querySelectorAll('.slider-before-after');
    sliders.forEach(function (slider) {
        setup({ el: slider });
    });
}

function autoRunAfter() {
    var sliders = document.querySelectorAll('.slider-before-after');
    sliders.forEach(function (slider) {
        setupAfter({ el: slider });
    });
}
...
var sliderSection = document.querySelector('.grants__slider__inner');

_swiper2.default.use([_swiper.Navigation, _swiper.Pagination, _swiper.Autoplay, _swiper.EffectFade, _swiper.Parallax]);

// const sliderLeft = document.querySelector('.navi_arrows__left');
// const sliderRight = document.querySelector('.navi_arrows__right');
...
var sliderSectionContent = sliderSection.querySelector('.grants__slider__inner__img');
var sliderSectionImg = sliderSection.querySelector('.grants__slider__inner__text-slider');
var sliderSectionTitle = sliderSection.querySelector('.left_part__text__title');

var tabsContainer = sliderSection.querySelector('.slider-pagi');

var progressTabs = tabsContainer.querySelectorAll('.slider-pagi__item--progress');
...
var changeContentSlide = function changeContentSlide(newIndex) {
    ...
    var activeClassName = 'grants__slider__inner__text-slider__slide--active';
    var slides = sliderSection.querySelectorAll('.grants__slider__inner__text-slider__slide');
    var activeSlide = sliderSection.querySelector('.' + activeClassName);

    var lines = document.querySelectorAll('.slider-pagi__item');
    var linesActive = 'slider-pagi__item--active';
    var lineActive = document.querySelector('.' + linesActive);

    var linesFill = 'slider-pagi__item--fill';
    var linesFillActive = document.querySelectorAll('.' + linesFill);

    var leftSliderValue = sliderSection.querySelector('.pagi_block__left');

    //  Смена цифры слайдера

    leftSliderValue.textContent = "0" + Number(newIndex + 1);
    ...
    if (lineActive) {
        lineActive.classList.remove(linesActive);
    }

    slides[newIndex].classList.add(activeClassName);
    lines[newIndex].classList.add(linesActive);

    if (linesFillActive) {
        for (var i = 0; i < linesFillActive.length; i++) {
            linesFillActive[i].classList.remove(linesFill);
        }
    }

    switch (newIndex) {
        case 0:
            // sliderLeft.classList.add('navi_arrows__left--disable');
            // sliderRight.classList.remove('navi_arrows__right--disable');
            break;
        case 1:
            lines[0].classList.add(linesFill);
            // sliderLeft.classList.remove('navi_arrows__left--disable');
            // sliderRight.classList.remove('navi_arrows__right--disable');
            break;
        case 2:
            lines[0].classList.add(linesFill);
            lines[1].classList.add(linesFill);
            // sliderRight.classList.add('navi_arrows__right--disable');
            // sliderLeft.classList.remove('navi_arrows__left--disable');
            break;
    }
};

var mainSlider = new _swiper2.default('.grants__slider__inner__img', {
    ...
    navigation: {
        nextEl: '.navi_arrows__right, .pagi_block__right',
        prevEl: '.navi_arrows__left, .pagi_block__left'
    },
    ...
    on: {
        ...
        slideChangeTransitionStart: function slideChangeTransitionStart() {
            if (mainSlider) {
                changeContentSlide(mainSlider.realIndex);
            }
        },
...
var sliderMouseHandler = function sliderMouseHandler(evt) {
    ...
    var pauseClassName = 'slider-pagi__item--active--paused';
    ...
    var sanimationEndHandler = function sanimationEndHandler(el) {
    ...
    if (isEventStart && !isContainsClass) {
        var _loop = function _loop(i) {
            var el = progressTabs[i];
            el.addEventListener('animationend', function () {
                sanimationEndHandler(el);
            });
        };
...
sliderSectionImg.addEventListener('mouseenter', sliderMouseHandler);
sliderSectionImg.addEventListener('mouseleave', sliderMouseHandler);

sliderSectionTitle.addEventListener('mouseenter', sliderMouseHandler);
sliderSectionTitle.addEventListener('mouseleave', sliderMouseHandler);
