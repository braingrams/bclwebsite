  ...
  [59],
  {
      ...
      function onScroll() {
        ...
        parallaxElements.forEach(function (parallaxElement) {
          ...
          parallaxElement.domElement.style.transform =
            "translate3d(0, " +
            elementTranslateY * parallaxElement.speed +
            "px, 0)";
        });
      }
    ...
    /***/ 496: /***/ function (module, exports, __webpack_require__) {
      ...
      function truncate(node) {
        ...
        node.textContent = str.substr(0, MAX_LENGTH - 1) + "...";
      }

      function retrieve(node) {
        var template = node.parentNode.querySelector(
          ".see-also__card__template"
        );
        ...
        var nodeFragment = template.content.cloneNode(true);
        var nodeFragmentStr = nodeFragment.textContent;
        var str = node.textContent;

        if (str.length === nodeFragmentStr.length) {
          return;
        }

        node.textContent = nodeFragmentStr;
      }

      function init() {
        var descriptionNodeList = document.querySelectorAll(
          ".see-also__card__description > span"
        );

        for (var i = 0; i < descriptionNodeList.length; i++) {
          var descriptionNode = descriptionNodeList[i];

          if (!view.isTablet()) {
            retrieve(descriptionNode);
          } else {
            truncate(descriptionNode);
          }
        }
      }
