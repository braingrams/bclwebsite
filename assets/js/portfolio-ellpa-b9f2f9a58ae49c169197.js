...
window.conversionGoal = 'ellpa_conversion_goal';
 ...
 */
function initHandlers() {
	var elements = Array.apply(null, document.querySelectorAll('.js-request_estimate_button'));
	var defTitle = requestEstimateForm.getTitle();

	elements.forEach(function (element) {
		element.addEventListener('click', function () {
			googleAnalytics.virtualPage({ page: '/feedback_footer' });
			requestEstimatePopup.show(this);
			requestEstimateForm.setTitle('Get in Touch');
		});
	});

	requestEstimatePopup.on("hide", function () {
		requestEstimateForm.setTitle(defTitle);
	});
}

common.init();
requestEstimatePopup.init();
requestEstimateForm.init(requestEstimatePopup.getDOM());
footer.init();
initHandlers();
