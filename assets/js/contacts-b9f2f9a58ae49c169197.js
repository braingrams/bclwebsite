...
window.conversionGoal = 'contacts_conversion_goal';
...
var contactsMap = __webpack_require__(1594);
...
function initHandlers() {
	...
	var emailLink = document.querySelectorAll(".l-mail-link.b-contacts_mail_link");
 ...
 */
function onResize() {
	!viewport.isMobile() ? disableMobileHandlers() : enableMobileHandlers();
}
...
function enableMobileHandlers() {
	contactsMap.on("request_click", requestEstimateShow);
}
...
function disableMobileHandlers() {
	contactsMap.off("request_click", requestEstimateShow);
}
...
var Handlers = __webpack_require__(7);
var viewport = __webpack_require__(20);
var GoogleMap = __webpack_require__(1595);
...
var city = {
  ...
  additional: "atlanta",
  new: "palmcity",
  additional_cleveland: "cleveland",
  additional_chicago: "chicago"
};
var currentOffice = "";
var DOM = {};
var handlers = new Handlers();
var settings = {
  desktop: {
    center: { lat: 30.5989946, lng: -76.9674532 },
    zoom: 5
  },
  portrait: {
    center: { lat: 30.5989946, lng: -76.2674532 },
    zoom: 5
  },
  landscape: {
    center: { lat: 30.5989946, lng: -76.9674532 },
...
module.exports = {
  init: function init() {
    ...
    initHandlers();
    createMap(function () {
      createOffice("additional_cleveland");
      createOffice("headquarters");
      createOffice("additional");
      createOffice("new");
      createOffice("additional_chicago");
...
function setDOM() {
  ...
  DOM.info = document.querySelector(".js-contacts_info");

  DOM.mobile = {
    request: document.querySelector(".js-contacts_request")
  };
...
function onResize() {
  ...
  if (didOrientationChange()) {
    Contacts.GoogleMap.refresh(!viewport.isMobile() ? settings.desktop : viewport.isPortrait() ? settings.portrait : settings.landscape);
    updateViewClasName();
  }
}
 ...
 */
function enableMobileHandlers() {
  DOM.mobile.request.addEventListener("click", onMobileRequestClick);
}
 ...
 */
function disableMobileHandlers() {
  DOM.mobile.request.removeEventListener("click", onMobileRequestClick);
}

/**
 * @private
 */
function onMobileRequestClick() {
  handlers.call("request_click");
}
...
function createMap(callback) {
  Contacts.GoogleMap = new GoogleMap("contacts_map");

  Contacts.GoogleMap.init(function () {
    Contacts.GoogleMap.createMap(!viewport.isMobile() ? settings.desktop : viewport.isPortrait() ? settings.portrait : settings.landscape);

    if (!Contacts.GoogleMap.map) {
...
function createMarker(office) {
  Contacts.Marker[office] = Contacts.GoogleMap.createMarker(city[office]);

  if (!Contacts.Marker[office]) {
    throw new Error("Marker is not created for " + city[office]);
  }
}
 ...
 * @param office {String}
 */
function createInfobox(office) {
  Contacts.Infobox[office] = Contacts.GoogleMap.createInfobox({
    content: getContent(office)
  });

  if (!Contacts.Infobox[office]) {
    throw new Error("Infobox is not created for " + office);
  }

  Contacts.Infobox[office].open(Contacts.GoogleMap.map, Contacts.Marker[office]);
}
 ...
 * @returns {HTMLElement}
 */
function getContent(office) {
  if (!Contacts.Content.hasOwnProperty(office)) {
    Contacts.Content[office] = document.querySelector(".js-contacts_infobox." + office);
  }

  return Contacts.Content[office];
}
 ...
 * @param office {String}
 */
function addHandlers(office) {
  window.addEventListener("resize", function () {
    Contacts.GoogleMap.refresh();
  });

  Contacts.GoogleMap.on(Contacts.Marker[office], "click", function () {
    toggleInfo(office);
  });

  getContent(office).onclick = function () {
    toggleInfo(office);
  };
}
 ...
 */
function initMap() {
  currentOffice = "additional";

  var defaultOffice = "new";

  toggleInfo(defaultOffice);
  Contacts.GoogleMap.selectMarker(Contacts.Marker[defaultOffice]);

  DOM.info.setAttribute("style", null);

  setTimeout(function () {
    DOM.info.classList.remove("inactive");
  }, 500);
}
 ...
 * @param office {String}
 */
function toggleInfo(office) {
  if (currentOffice !== office) {
    hideInfo(currentOffice);
    showInfo(office);
  }

  DOM.info.classList.remove("b-contacts_info_closed");
}

/**
 * @private
 */
function hideInfo(office) {
  if (!office) {
    return;
  }

  DOM.info.classList.remove(office);
  getContent(office).classList.remove("inactive");
}
...
function showInfo(office) {
  if (!DOM.info.classList.contains(office)) {
    currentOffice = office;
    DOM.info.classList.add(office);
    getContent(currentOffice).classList.add("inactive");
  }

  DOM.info.classList.remove("b-contacts_info_closed");
}

function didOrientationChange() {
  var html = DOM.html;
  var hasLandscapeClassName = html.classList.contains("landscape");
  var hasPortraitClassName = html.classList.contains("portrait");

  var orientationChange = viewport.isDesktop() && (hasPortraitClassName || hasLandscapeClassName);

  if (viewport.isMobile()) {
    orientationChange = orientationChange || viewport.isPortrait() && !hasPortraitClassName || viewport.isLandscape() && !hasLandscapeClassName;
  }

  return orientationChange;
}

function updateViewClasName() {
  DOM.html.classList.remove("portrait");
  DOM.html.classList.remove("landscape");

  if (viewport.isMobile()) {
    DOM.html.classList.add(viewport.isLandscape() ? "landscape" : "portrait");
  }
}
...
var _point = __webpack_require__(1596);

var _point2 = _interopRequireDefault(_point);

var _pointGray = __webpack_require__(1597);

var _pointGray2 = _interopRequireDefault(_pointGray);
...
var GoogleMapsLoader = __webpack_require__(1598);
var InfoBox; // node module: google-maps-infobox

/**
 * @private
 */
var positions = {
	atlanta: { lat: 33.7388771, lng: -84.8956449 },
	miami: { lat: 25.7349006, lng: -80.7470946 },
	stuart: { lat: 27.1735918, lng: -80.7477789 },
	cleveland: { lat: 41.4993, lng: -81.9944 },
	chicago: { lat: 41.8760, lng: -88.1689 },
	palmcity: { lat: 27.1735918, lng: -80.7777789 }
};

var markers = [];
var markerIcon = null;
var markerActiveIcon = null;
 ...
 * @constructor
 */
var GoogleMap = function GoogleMap(wrapId) {
	if (!(this instanceof GoogleMap)) {
		return new GoogleMap(wrapId);
	}
 ...
 * @param callback {Function}
 */
GoogleMap.prototype.init = function (callback) {
	GoogleMapsLoader.SENSOR = false;
	GoogleMapsLoader.KEY = "AIzaSyCoqrXF350U3pT_oHHmMpPOq4pFwepUT4w";

	GoogleMapsLoader.load(function () {

		InfoBox = __webpack_require__(1599);
		modifyInfobox();
		this.setOptions();
		callback && callback();
	}.bind(this));
 ...
 * @public
 */
GoogleMap.prototype.setOptions = function () {
	this.opt.marker = extend({}, this.opt.marker, {
		icon: markerIcon
	});
 ...
 * @param opt {Object}
 */
GoogleMap.prototype.createMap = function (opt) {
	opt = opt || {};
	this.opt.map = extend({}, this.opt.map, opt);
	this.map = new google.maps.Map(this.wrap, this.opt.map);
 ...
 */
GoogleMap.prototype.createMarker = function (pos, opt) {
	opt = opt || {};
	opt.position = this.getPosition(pos);
	opt.map = this.map;
	this.opt.marker = extend({}, this.opt.marker, opt);

	var marker = new google.maps.Marker(this.opt.marker);
 ...
 */
GoogleMap.prototype.createInfobox = function (opt) {
	var infobox = new InfoBox(extend({
		boxClass: "c-google_map_infobox_wrap",
		closeBoxURL_: ""
	}, opt));

	infobox.fixedWidthSet_ = false;
	this.on(infobox, "domready", function () {
		infobox.div_.style.width = "";
	});
 ...
 * @param callback {Function}
 */
GoogleMap.prototype.on = function (obj, event, callback) {
 ...
 * @param event {String}
 */
GoogleMap.prototype.trigger = function (obj, event) {
 ...
 * @param pos {Object}
 */
GoogleMap.prototype.refresh = function (pos) {
	if (typeof pos === "undefined") {
		return;
	}

	pos = pos || {};
	this.map.panTo(pos.center ? pos.center : this.opt.map.center);
	pos.zoom && this.map.setZoom(pos.zoom);
};
 ...
 * @param pos {String}
 */
GoogleMap.prototype.getPosition = function (pos) {
	if (!positions.hasOwnProperty(pos)) {
		throw new Error("Incorrect position '" + pos + "'");
	}
...
function modifyInfobox() {
	markerIcon = createMarkerIcon(_pointGray2.default);
	markerActiveIcon = createMarkerIcon(_point2.default);

	InfoBox.prototype.getCloseBoxImg_ = function () {
		// closeBoxURL_ - HTML element
		return typeof this.closeBoxURL_ === "string" || !this.closeBoxURL_ ? "<div></div>" : this.closeBoxURL_;
	};
...
module.exports = __webpack_require__.p + "assets/files/point-26b7cc4d15eeed2c1d58f1243ea77428.svg";
...
module.exports = __webpack_require__.p + "assets/files/point-gray-497af6acfdee38ddd32b5360f1be4b3c.svg";
