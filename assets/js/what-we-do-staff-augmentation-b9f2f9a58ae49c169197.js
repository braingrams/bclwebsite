...
module.exports = __webpack_require__(2149);
...
var _Carousel = __webpack_require__(2152);
...
var carousel = new _Carousel2.default("lasting-partnerships");
...
var Carousel = function () {
    function Carousel(id) {
        ...
        this._carouselContainerElement = document.getElementById(id);
        this._carouselElement = this._carouselContainerElement.querySelector(".landing__carousel");

        this._slideContainerElements = this._carouselElement.querySelectorAll(".landing__carousel-slide-container");

        if (this._slideContainerElements.length < 3) {
            throw new Error("Number of slides must be greater than 2.");
        }

        var numberOfSlides = this._slideContainerElements.length;

        this._rotationStepAngle = DEG_360 / numberOfSlides;

        this._slideContainers = [];

        var slideContainerStartAngle = DEG_90;

        for (var i = 0; i < this._slideContainerElements.length; i++) {
            this._slideContainers.push({
                angle: slideContainerStartAngle,
                element: this._slideContainerElements[i]
            });

            slideContainerStartAngle += this._rotationStepAngle;
        }

        this._previousSlideButtonElement = this._carouselContainerElement.querySelector(".landing__carousel-button--prev");
        this._nextSlideButtonElement = this._carouselContainerElement.querySelector(".landing__carousel-button--next");
        ...
        setTimeout(function () {
            _this._carouselContainerElement.classList.add("landing__carousel-transition");
        }, 0);
    }

    _createClass(Carousel, [{
        key: "handleNextSlideButtonClick",
        value: function handleNextSlideButtonClick() {
            this._rotationDirection = -1;
            this._rotationAngle += this._rotationStepAngle;

            this.updateSlides();
        }
    }, {
        key: "handlePreviousSlideButtonClick",
        value: function handlePreviousSlideButtonClick() {
            this._rotationDirection = 1;
            this._rotationAngle -= this._rotationStepAngle;
        ...
        value: function handleResize() {
            if (window.matchMedia("(min-width: 1025px)").matches) {
                if (!this._isDesktop) {
                    ...
                    this._previousSlideButtonElement.addEventListener("click", this.handlePreviousSlideButtonClick, false);
                    this._nextSlideButtonElement.addEventListener("click", this.handleNextSlideButtonClick, false);
                }

                this._carouselContainerElement.classList.remove("landing__carousel-transition");
                this.updateSlides();
                ...
                if (this._isDesktop) {
                    ...
                    this._previousSlideButtonElement.removeEventListener("click", this.handlePreviousSlideButtonClick, false);
                    this._nextSlideButtonElement.removeEventListener("click", this.handleNextSlideButtonClick, false);
                }
            }
        }
    }, {
        ...
        value: function updateSlides() {
            ...
            for (var i = 0; i < this._slideContainers.length; i++) {
                var slideContainer = this._slideContainers[i];
                slideContainer.element.childNodes[0].classList.remove("landing__carousel-slide--current");

                var angle = slideContainer.angle + this._rotationAngle;
                ...
                var additionalZIndex = 0;

                if (this._rotationDirection < 0 && x < 0 || this._rotationDirection > 0 && x > 0) {
                    additionalZIndex = 1;
                }

                slideContainer.element.style.transform = "translate3d(" + (x * this._carouselElement.offsetWidth / 2 + this._carouselElement.offsetWidth / 2) + "px, 0, 0)";
                slideContainer.element.style.zIndex = Math.round((y + 1) * 100 + additionalZIndex);
            }

            this._slideContainers[currentSlideIndex].element.childNodes[0].classList.add("landing__carousel-slide--current");
        }
    }]);
...
_swiper2.default.use({ Pagination: _swiper.Pagination, Autoplay: _swiper.Autoplay });

var view = _View2.default.getInstance();

var section = document.querySelector('.landing__slider-section');
var slider = section.querySelector('.slider-industries');

var industriesSlider = void 0;
var clientWidth = document.body.clientWidth;


var sliderInit = function sliderInit() {
    if (!view.isDesktop()) {
        industriesSlider = new _swiper2.default(slider, {
            mode: 'horizontal',
            speed: 250,
            slidesPerView: 'auto',
            spaceBetween: 16,
            ...
            pagination: {
                bulletActiveClass: 'staff-augmentation__bullets__item--active',
                bulletClass: 'staff-augmentation__bullets__item',
                clickable: true,
                el: '.staff-augmentation__bullets--industries',
                type: 'bullets'
            }
        });
    }
...
var initAutoPlay = function initAutoPlay() {
    if (view.isDesktop() || !industriesSlider || industriesSlider.autoplay.running) {
        return;
    }

    var current = industriesSlider.wrapperEl;

    while (!current.classList.contains('landing__slider-section')) {
    ...
    if (bounding.top < 400) {
        industriesSlider.autoplay.start();
        industriesSlider.params.autoplay.delay = 5000;
        industriesSlider.params.autoplay.disableOnInteraction = false;
    }
};
...
var tabNames = ['Idea', 'Requirements Analysis', 'Design', 'Development', 'Quality Assurance', 'Launch'];

var paginationElement = document.querySelector('.staff-augmentation__our-process__swiper-pagination');
var sliderSection = document.querySelector('.staff-augmentation__our-process');
var sliderSectionContent = sliderSection.querySelector('.staff-augmentation__our-process__swiper-block');
var tabsContainer = sliderSection.querySelector('.staff-augmentation__our-process__tabs-container');
var sliderOurProcess = sliderSection.querySelector('.staff-augmentation__our-process__swiper-container');
var sliderOurProcessMobile = sliderSection.querySelector('.staff-augmentation__our-process__swiper-container--content');

var ourProcessSlider = void 0;
var clientWidth = document.body.clientWidth;

var currentDevice = null;
var progressTabs = void 0;
var tabElements = void 0;

var getCurrentDevice = function getCurrentDevice() {
    if (view.isDesktop()) {
        currentDevice = 'desktop';
    } else if (view.isTablet()) {
...
var changeContentSlideOurProcess = function changeContentSlideOurProcess(newIndex) {
    if (typeof newIndex === 'undefined') {
        return;
    }

    var activeContentSlideClassName = 'staff-augmentation__our-process__swiper-container--content__swiper-slide--active';
    var slides = document.querySelectorAll('.staff-augmentation__our-process__swiper-container--content__swiper-slide');
    var activeSlide = document.querySelector('.' + activeContentSlideClassName);

    if (activeSlide) {
        activeSlide.classList.remove(activeContentSlideClassName);
    }

    slides[newIndex].classList.add(activeContentSlideClassName);
};

var initTabsAnimation = function initTabsAnimation(tabElements) {
    if (tabElements.length) {
        paginationElement.classList.add('staff-augmentation__our-process__swiper-pagination--loaded');
    }
};

var clearedTabsLoading = function clearedTabsLoading() {
    if (paginationElement.classList.contains('staff-augmentation__our-process__swiper-pagination--loaded')) {
        paginationElement.classList.remove('staff-augmentation__our-process__swiper-pagination--loaded');
    }
};

var showMobileBtns = function showMobileBtns(index) {
    var mobileBtns = sliderSection.querySelectorAll('.staff-augmentation__btn__our-process');
    var activeBtnClassName = 'staff-augmentation__btn__our-process--active';

    for (var i = 0; i < mobileBtns.length; i++) {
        var el = mobileBtns[i];
        var elIndex = parseInt(el.dataset.btn, 10) - 1;

        if (elIndex === index) {
            el.classList.add(activeBtnClassName);
        } else if (el.classList.contains(activeBtnClassName)) {
...
var sliderInit = function sliderInit() {
    if (currentDevice === 'desktop') {
        ourProcessSlider = new _swiper2.default(sliderOurProcess, {
            loop: true,
            ...
            navigation: {
                nextEl: '.staff-augmentation__our-process__swiper-button-next',
                prevEl: '.staff-augmentation__our-process__swiper-button-prev'
            },

            pagination: {
                clickable: true,
                el: '.staff-augmentation__our-process__swiper-pagination',

                renderBullet: function renderBullet(index, className) {
                    return '\n                        <div class=\'staff-augmentation__our-process__tab swiper-pagination-bullet\'>\n                            <div class=\'staff-augmentation__our-process__tab__line staff-augmentation__our-process__tab__line--default\'></div>\n                            <span>' + tabNames[index] + '</span>\n                            <div class=\'staff-augmentation__our-process__tab__line staff-augmentation__our-process__tab__line--progress\'></div>\n                        </div>\n                    ';
                }
            },

            on: {
                init: function init() {
                    changeContentSlideOurProcess(0);
                },

                slideChangeTransitionStart: function slideChangeTransitionStart() {
                    if (ourProcessSlider) {
                        changeContentSlideOurProcess(ourProcessSlider.realIndex);
                    }
                },

                paginationRender: function paginationRender() {
                    progressTabs = tabsContainer.querySelectorAll('.staff-augmentation__our-process__tab__line--progress');
                }
            }
        });
    } else if (currentDevice === 'tablet') {
        ourProcessSlider = new _swiper2.default(sliderOurProcessMobile, {
            loop: true,
            loopAdditionalSlides: 2,
        ...
        ourProcessSlider = new _swiper2.default(sliderOurProcessMobile, {
            ...
            pagination: {
                ...
                bulletClass: 'staff-augmentation__bullets__item',
                clickable: true,
                el: '.staff-augmentation__bullets--our-process',
                type: 'bullets'
            },
            ...
            on: {
                ...
                slideChangeTransitionStart: function slideChangeTransitionStart() {
                    if (ourProcessSlider) {
                        showMobileBtns(ourProcessSlider.realIndex);
                    }
                },

                paginationRender: function paginationRender() {
                    progressTabs = sliderSectionContent.querySelectorAll('.staff-augmentation__our-process__swiper-container--content__tab__line--progress');
                }
            }
        });
    }
...
var initAutoPlay = function initAutoPlay() {
    var current = document.querySelector('.staff-augmentation__our-process');
    var bounding = current.getBoundingClientRect();
    tabElements = tabsContainer.querySelectorAll('.staff-augmentation__our-process__tab');
...
var resizeHandlerSlider = function resizeHandlerSlider() {
    if (clientWidth !== document.body.clientWidth && currentDevice !== getCurrentDevice()) {
        clientWidth = document.body.clientWidth;
        tabElements = document.querySelectorAll('.staff-augmentation__our-process__tab');

        view._resize();

        if (ourProcessSlider) {
            ourProcessSlider.destroy();
        }
...
var isMobileScrolling = function isMobileScrolling(evt) {
    if (evt.type !== 'attributes' || !document.querySelector('.touchevents')) {
        return false;
    }

    return evt.oldValue === 'disable-pointer-events' ? 'stop' : 'start';
};

var sliderMouseHandler = function sliderMouseHandler(evt) {
    var type = evt.type;

    var pauseClassName = 'staff-augmentation__our-process__tabs-container--paused';
    var isContainsClass = tabsContainer.classList.contains(pauseClassName);
    var isEventStart = true ? type === 'mouseenter' || isMobileScrolling(evt) === 'start' : type === 'mouseleave' || isMobileScrolling(evt) === 'stop';

    if (isEventStart && !isContainsClass) {
        for (var i = 0; i < progressTabs.length; i++) {
            progressTabs[i].addEventListener('animationend', function () {
                ourProcessSlider.slideNext();
                ourProcessSlider.autoplay.start();
            });
        }

        currentDevice === 'phone' && sliderSectionContent.classList.add(pauseClassName);
        tabsContainer.classList.add(pauseClassName);
        ourProcessSlider.autoplay.stop();
    } else if (!isEventStart && isContainsClass) {
        tabsContainer.classList.remove(pauseClassName);
        currentDevice === 'phone' && sliderSectionContent.classList.remove(pauseClassName);
    }
};

sliderSectionContent.addEventListener('mouseenter', sliderMouseHandler);
sliderSectionContent.addEventListener('mouseleave', sliderMouseHandler);
...
var view = _View2.default.getInstance();
var section = document.querySelector('.staff-augmentation__partnership');
var slider = section.querySelector('.landing__carousel-container--mobile--images');
...
var changeContentSlide = function changeContentSlide(newIndex) {
    ...
    var activeClassName = 'landing__carousel-container--mobile--content__item--active';
    var slides = section.querySelectorAll('.landing__carousel-container--mobile--content__item');
...
var sliderInit = function sliderInit() {
    if (view.isMobile()) {
        swiperCarousel = new _swiper2.default(slider, {
            ...
            navigation: {
                nextEl: '.landing__carousel-container--mobile__swiper-button-next',
                prevEl: '.landing__carousel-container--mobile__swiper-button-prev'
            },
            ...
            pagination: {
                ...
                bulletClass: 'staff-augmentation__bullets__item',
                clickable: true,
                el: '.staff-augmentation__bullets--lasting-partnerships',
                type: 'bullets'
            },
...
var sliderSectionAwardsЕstimonials = document.querySelector('.staff-augmentation__awards-testimonials__swiper-inner');

var clientWidth = document.body.clientWidth;

var awardsЕstimonialsSlider = void 0;

// ToDo
var changeContentSlideAwardsЕstimonials = function changeContentSlideAwardsЕstimonials(newIndex) {
    if (typeof newIndex === 'undefined') {
        return;
    }

    var activeClassName = 'staff-augmentation__awards-testimonials__content-blocks__item--active';
    var slides = sliderSectionAwardsЕstimonials.querySelectorAll('.staff-augmentation__awards-testimonials__content-blocks__item');
    var activeSlide = sliderSectionAwardsЕstimonials.querySelector('.' + activeClassName);
...
var sliderInit = function sliderInit() {
    if (view.isDesktop()) {
        awardsЕstimonialsSlider = new _swiper2.default(sliderSectionAwardsЕstimonials.querySelector('.staff-augmentation__awards-testimonials__swiper-container'), {
            loop: true,
            ...
            on: {
                init: function init() {
                    changeContentSlideAwardsЕstimonials(0);
                },

                slideChangeTransitionStart: function slideChangeTransitionStart() {
                    if (awardsЕstimonialsSlider) {
                        changeContentSlideAwardsЕstimonials(awardsЕstimonialsSlider.realIndex);
                    }
                }
            },

            navigation: {
                nextEl: '.staff-augmentation__awards-testimonials__swiper-button-next',
                prevEl: '.staff-augmentation__awards-testimonials__swiper-button-prev'
            }
        });
    } else if (view.isMobile()) {
        awardsЕstimonialsSlider = new _swiper2.default(sliderSectionAwardsЕstimonials.querySelector('.staff-augmentation__awards-testimonials__content-blocks'), {
            mode: 'horizontal',
            loop: true,
            speed: 250,
            spaceBetween: 32,
            ...
            pagination: {
                ...
                bulletClass: 'staff-augmentation__bullets__item',
                clickable: true,
                el: '.staff-augmentation__bullets--awards-testimonials',
                type: 'bullets'
            }
        });
    }
...
var resizeHandlerSlider = function resizeHandlerSlider() {
    if (clientWidth !== document.body.clientWidth) {
        ...
        if (awardsЕstimonialsSlider) {
            awardsЕstimonialsSlider.destroy();
        }
...
var anchorLinkScrollHandler = function anchorLinkScrollHandler() {
    currentOffset = window.scrollY;
    ratio = currentOffset / viewportHeight;

    if (!anchorLink || currentOffset > viewportHeight) {
        return;
    }

    var opacityValue = ratio / 0.25 - 1;
    ...
    anchorLink.style.opacity = opacityValue;
};

window.addEventListener('scroll', anchorLinkScrollHandler);
