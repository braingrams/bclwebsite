...
module.exports = __webpack_require__(1980);
...
var _aos = __webpack_require__(745);
...
var _toggle_mouse_event = __webpack_require__(114);

var _toggle_mouse_event2 = _interopRequireDefault(_toggle_mouse_event);

var _swiper = __webpack_require__(128);

var _swiper2 = _interopRequireDefault(_swiper);

var _what_we_do_parallax_for_axiosY_exported = __webpack_require__(746);

var _what_we_do_parallax_for_axiosY_exported2 = _interopRequireDefault(_what_we_do_parallax_for_axiosY_exported);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

_swiper2.default.use([_swiper.Navigation, _swiper.Pagination, _swiper.Autoplay]);

var section = document.querySelector('.tonal--reviews');
...
var container = document.querySelectorAll(".tonal--reviews--card--top");

swiperCarousel.on("slideChange", function () {
    var _this = this;

    container.forEach(function (slide) {
        if (slide.dataset.tstnumber == _this.activeIndex || slide.dataset.tstnumbersec && slide.dataset.tstnumbersec == _this.activeIndex) slide.style.display = "flex";else slide.style.display = "none";
    });
});

_common2.default.init();
_footer2.default.init();
_toggle_mouse_event2.default.init();

window.addEventListener("load", function () {
    (0, _what_we_do_parallax_for_axiosY_exported2.default)();
    _aos2.default.init();
});
...
function init() {
  ...
  if (domElements.length) {
    ...
    if (svgElements.length) {
      for (var _i = 0; _i < svgElements.length; _i++) {
        var svgElement = svgElements[_i];

        var maskHeight = svgElement.clientHeight;
        var patternHeight = svgElement.querySelector('pattern').querySelector('image').getBoundingClientRect().height;
        var patternImage = svgElement.querySelector('pattern').querySelector('image');

        var speed = svgElement.dataset.speed;
        var windowScrollY = window.innerHeight / 2 + window.scrollY;
        var patternY = -(maskHeight / patternHeight * 100 + 100);

        if (patternHeight <= maskHeight * 2) {
          patternY = -(maskHeight / patternHeight * 100);
        }

        patternImage.setAttribute('y', patternY + windowScrollY * speed + '%');

        parallaxSvgElements.push({
          domElement: svgElement,
          speed: parseFloat(svgElement.dataset.speed) || 0
        });
      }
    }
...
function onScroll() {
  ...
  parallaxSvgElements.forEach(function (parallaxSvgElement) {
    // const elementTranslateY = (parallaxSvgElement.domElement.getBoundingClientRect().top - windowScrollY);
    // const deltaY = elementTranslateY * parallaxSvgElement.speed;

    var maskHeight = parallaxSvgElement.domElement.clientHeight;
    var patternHeight = parallaxSvgElement.domElement.querySelector('pattern').querySelector('image').getBoundingClientRect().height;
    var patternImage = parallaxSvgElement.domElement.querySelector('pattern').querySelector('image');

    var speed = parallaxSvgElement.domElement.dataset.speed;
    var windowScrollY = window.innerHeight / 2 + window.scrollY;
    var patternY = -(maskHeight / patternHeight * 100 + 100);
    ...
    if (window.innerHeight >= parallaxSvgElement.domElement.getBoundingClientRect().top) {
      var deltaY = patternY + windowScrollY * speed;
      patternImage.setAttribute('y', deltaY.toFixed(2) + '%');
    }
  });
}
